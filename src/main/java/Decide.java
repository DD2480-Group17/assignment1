...
public class Decide {
	 ...
	 *         > (PI + EPSILON) The second of the three consecutive points is always
	 *         the vertex of the angle. If either the first point or the last point
	 *         (or both) coincides with the vertex, the angle is undefined and false
	 *         is returned
	 */
	boolean lic2() {
	...
	/**
	 * lic4 checks if it exists QPTS consecutive data points in more then QUADS
	 * unique quadrants, where , the data point (0,0) is in quadrant I, the point
	 * (-l,0) is in quadrant II, the point (0,-l) is in quadrant III, the point
	 * (0,1) is in quadrant I and the point (1,0) is in quadrant I *
	 *
	 * @return the function returns true if it exists QPTS consecutive data points
	 *         in more then QUADS unique quadrants
	 */
	boolean lic4() {
		// checks if qPts or quads is in a undefined area for this function

		if (parameters.qPts < 2 || parameters.qPts > points.size() || parameters.quads < 1 || parameters.quads > 3) {
			return false;
		}
	 ...
	 *
	 * @return true if there exists one set of two consecutive data points such that
	 *         X[j]-X[j-1] < 0
	 */
	boolean lic5() {
	 ...
	 *         exactly kPts consecutive intervening points that are a distance
	 *         greater than the length,LENGTH1, apart. The condition is not met when
	 *         numPoints < 3.
	 */
	boolean lic7() {
	 ...
	 *
	 * @return if there exists at least one set of three data points separated by
	 *         exactly A PTS and B PTS consecutive intervening points, respectively,
	 *         that cannot be contained within or on a circle of radius RADIUS1. The
	 ...
	 * @return true, if the three points can not be contained in a circle of radius
	 *         r
	 */
	...
	/**
	 * Calculates the Preliminary Unlocking Matrix (PUM) LCM[i,j] represents the
	 * boolean operator to be applied to CMV[i] and CMV[j]. PUM[i,j] is set
	 * according to the result of this operation. If LCM[i,j] is NOTUSED, then
	 * PUM[i,j] should be set to true. If LCM[i,j] is ANDD, PUM[i,j] should be set
	 * to true only if (CMV[i] AND CMV[j]) is true. If LCM[i,j] is ORR, PUM[i,j]
	 * should be set to true if (CMV[i] OR CMV[j]) is true.
	 */
	void calcPum() {
	 ...
	 *         respectively, that are the vertices of a triangle with area greater
	 *         than AREA1
	 */
	boolean lic10() {
		if (points.size() < 5)
		...
		for (int i = 0; i + parameters.ePts + parameters.fPts + 2 < points.size(); i++) {
			if (triangleArea(points.get(i), points.get(i + parameters.ePts + 1),
					points.get(i + parameters.ePts + parameters.fPts + 2)) > parameters.area1) {
				return true;
			}
		}
		return false;
	 ...
	 *
	 * @return ture if there exists at least one set of two data points, (X[i],Y[i])
	...
	double triangleArea(Point2D.Double point0, Point2D.Double point1, Point2D.Double point2) {
		return Math
				.abs((point0.getX() * (point1.getY() - point2.getY()) + point1.getX() * (point2.getY() - point0.getY())
	 ...
	 * 
	 * @return true if there exists at least one set of two data points, separated
	 *         by exactly kPts consecutive intervening points, which are a distance
	 *         greater than the length1, apart AND there exists at least one set of
	 *         two data points, separated by exactly kPts consecutive intervening
	 *         points, that are a distance less than the length2, apart. The
	 ...
	 * are the vertices of a triangle with area greater than AREA1. In addition,
	 * there exist three data points (which can be the same or different from the
	 * three data points just mentioned) separated by exactly E PTS and F PTS
	 * consecutive intervening points, respectively, that are the vertices of a
	 * triangle with area less than AREA2. Both parts must be true for the LIC to be
	 * true. Otherwise, return false. Also, return false when NUMPOINTS < 5.
	 * 
	 ...
	 *         respectively, that are the vertices of a triangle with area greater
	 *         than AREA1. In addition, there exist three data points (which can be
	 *         the same or different from the three data points just mentioned)
	 ...
	 *         AREA2. Both parts must be true for the LIC to be true. Otherwise,
	 *         return false. Also, return false when NUMPOINTS < 5.
	 */
	boolean lic14() {
	...
	/**
	 * Calculates entries in FUV vector.for all i, FUV[i] is set to true if PUV[i]
	 * is false (indicating that the associated LIC should not hold back launch) or
	 * if for all elements i,j, i!=j, in PUM row i are true.
	 */
	void calcFUV() {
